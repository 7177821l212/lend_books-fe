export const translations = {
  en: {
    // Navigation
    nav_today: 'Today',
    nav_customers: 'Customers',
    nav_history: 'History',
    nav_me: 'Me',
    nav_home: 'Home',
    nav_team: 'Team',
    nav_reports: 'Reports',
    nav_settings: 'Settings',

    // Auth
    welcome_back: 'Welcome back',
    sign_in_to_continue: 'Sign in to continue',
    email: 'Email',
    password: 'Password',
    sign_in: 'Sign in',
    sign_out: 'Sign out',

    // Common
    save: 'Save',
    cancel: 'Cancel',
    edit: 'Edit',
    create: 'Create',
    delete: 'Delete',
    confirm: 'Confirm',
    loading: 'Loading…',
    error: 'Something went wrong',
    no_data: 'No data',
    back: 'Back',
    close: 'Close',
    refresh: 'Refresh',
    search: 'Search',
    filter: 'Filter',
    all: 'All',

    // Loan
    loan: 'Loan',
    loans: 'Loans',
    new_loan: 'New loan',
    loan_amount: 'Loan amount',
    principal: 'Principal',
    interest: 'Interest',
    disbursed: 'Disbursed',
    repayable: 'Repayable',
    profit: 'Profit',
    outstanding: 'Outstanding',
    installment: 'Installment',
    installments: 'Installments',
    start_date: 'Start date',
    frequency: 'Frequency',
    model_a: 'Model A · Pre-deduct',
    model_b: 'Model B · Add-on',
    create_loan: 'Create loan',
    close_loan: 'Close loan',
    loan_closed: 'Loan closed',
    assign_collector: 'Assign collector',
    reassign: 'Reassign',
    collections: 'Collections',
    schedule: 'Schedule',
    status_active: 'Active',
    status_overdue: 'Overdue',
    status_closed: 'Closed',
    status_cancelled: 'Cancelled',

    // Customer
    customer: 'Customer',
    customers: 'Customers',
    new_customer: 'New customer',
    full_name: 'Full name',
    phone: 'Phone',
    address: 'Address',
    risk_level: 'Risk level',
    risk_low: 'Low',
    risk_medium: 'Medium',
    risk_high: 'High',
    blacklist: 'Blacklist',
    unblacklist: 'Remove blacklist',
    blacklisted: 'Blacklisted',
    blacklist_reason: 'Blacklist reason',
    documents: 'Documents',
    upload: 'Upload',
    id_proof: 'ID Proof',
    address_proof: 'Address Proof',
    agreement: 'Signed Agreement',
    other: 'Other',

    // Payments
    collect: 'Collect',
    collected: 'Collected',
    missed: 'Missed',
    payment_mode: 'Payment mode',
    cash: 'Cash',
    upi: 'UPI',
    bank: 'Bank',
    amount: 'Amount',
    notes: 'Notes',
    why_missed: 'Why missed?',
    save_collection: 'Save collection',
    mark_as_missed: 'Mark as missed',
    customer_unavailable: 'Customer unavailable',
    customer_refused: 'Customer refused',
    shop_closed: 'Shop closed',
    promised_later: 'Promised later',
    wrong_address: 'Wrong address',
    medical_issue: 'Medical issue',

    // Dashboard
    net_outstanding: 'NET OUTSTANDING',
    today: 'TODAY',
    overdue: 'OVERDUE',
    top_collectors: 'Top collectors',
    capital_disbursed: 'Disbursed',
    total_collected: 'Collected',
    active_loans: 'Active loans',
    closed_loans: 'Closed',
    my_day: 'My Day',
    target: 'Target',
    pickups: 'Pickups',

    // Team
    team: 'Team',
    collectors: 'collectors',
    new_collector: 'New collector',
    add_collector: 'Add collector',
    visits: 'visits',
    collector_name: 'Full name',
    collector_email: 'Email',
    collector_phone: 'Phone (optional)',
    collector_password: 'Password',
    create_account: 'Create account',

    // Reports
    reports: 'Reports',
    insights: 'Insights',
    overdue_loans: 'Overdue loans',
    nothing_overdue: 'Nothing overdue',
    nobody_blacklisted: 'Nobody blacklisted',
    total_interest: 'Total interest',
    avg_loan: 'Avg loan',
    avg_rate: 'Avg rate',

    // Installment status
    status_pending: 'Pending',
    status_partial: 'Partial',
    status_due_today: 'Due today',
    status_missed: 'Missed',
    status_replaced: 'Replaced',

    // Schedule coverage
    planned: 'planned',
    advance_credit: 'advance credit',
    covered: 'covered',
    remaining: 'remaining',
    carried_forward: 'carried forward',
    record_collection: 'Record collection',
    total_to_collect: 'To collect',
    expected_per_visit: 'Expected / visit',
    since: 'since',
    missed_visits: 'Missed visits',
    collected_so_far: 'Collected',
    collection_date: 'Collection date',
    tap_a_bar: 'Tap a bar to see that day',
    collection_trend: 'Collection trend',
    last_30_days: 'Last 30 days',
    no_loans_assigned: 'No loans assigned yet',
    visit_date: 'Visit date',
    installments_too_many: 'Enter 10,000 installments or fewer',
    simple_loan: 'Simple loan',
    simple_loan_desc: 'No schedule \u2014 collect any amount, any day',
    collect_early_payment: 'Take an early payment',
    nothing_left_to_collect: 'This loan is fully collected',

    // Reschedule
    reschedule: 'Reschedule',
    reschedule_schedule: 'Reschedule remaining plan',
    reschedule_mode: 'How should the rest be repaid?',
    mode_same_end_date: 'Same end date',
    mode_same_end_date_desc: 'Finish on the agreed day; each visit collects less.',
    mode_same_installment: 'Same daily amount',
    mode_same_installment_desc: 'Keep the per-visit amount; the loan ends sooner.',
    reschedule_reason: 'Reason',
    reason_advance_payment: 'Advance payment',
    reason_customer_request: 'Customer request',
    reason_missed_payment: 'Missed payment',
    reschedule_preview: 'Preview changes',
    reschedule_preview_title: 'Old plan → New plan',
    reschedule_confirm: 'Confirm new schedule',
    reschedule_done: 'Schedule updated',
    reschedule_failed: 'Could not reschedule',
    enter_valid_amount: 'Enter an amount greater than zero',
    reschedule_note: 'Paid installments and collection records never change.',
    remaining_balance: 'Remaining balance',
    ends: 'Ends',
    revision_history: 'Revision history',
    revised_by: 'by',

    // My Day
    call: 'Call',
    all_caught_up: "You're all caught up",
    no_pickups_desc: 'No pickups due right now. Great work.',
    done: 'done',

    // History
    all_time: 'All time',
    entries: 'entries',
    no_entries_yet: 'No entries yet',
    your_collections_desc: 'Your collections will appear here.',

    // Status
    active: 'Active',
    inactive: 'Inactive',

    // Misc
    not_added: 'Not added',
    no_loans_yet: 'No loans yet',
    no_loans_desc: 'Loan history will appear here once you create one.',
    tap_to_view: 'Tap to view',
    missing: 'Missing',
    risk: 'Risk',
    new: 'New',
    no_collectors_yet: 'No collectors yet',
    no_collectors_desc: 'Tap + to add a collector and start assigning loans.',
    nothing_overdue_desc: 'Everything on schedule — nice.',
    nobody_blacklisted_desc: 'Customers will appear here when an investor blacklists them.',
    no_collections_yet: 'No collections yet',
    no_collections_desc: 'Performance will appear once collectors start recording payments.',
    lifetime: 'lifetime',
    needs_attention: 'needs attention',
    create_customer: 'Create customer',
    save_changes: 'Save changes',
    collector: 'Collector',
    name: 'Name',
    missed_visit: 'Missed visit',

    // Collector detail
    deactivate: 'Deactivate',
    activate: 'Activate',
    delete_collector: 'Delete collector',
    delete_customer: 'Delete customer',
    confirm_delete: 'Confirm delete',
    delete_collector_warn: 'This will permanently delete the collector. Reassign their active loans first.',
    delete_customer_warn: 'This will permanently delete the customer. Close their active loans first.',
    cannot_undo: 'This cannot be undone.',
    recovery_rate: 'Recovery rate',
    no_trend_data: 'No collections recorded yet',
    collection_rate: 'Collection rate',
    high_risk: 'High risk',
    sort_by: 'Sort',
    sort_amount: 'Amount',
    sort_installments: 'Installments',
    this_week: 'This week',
    this_month: 'This month',
    this_year: 'This year',
    period: 'Period',
    collector_filter: 'Collector',

    // Profile
    profile: 'Profile',

    // Settings
    settings: 'Settings',
    appearance: 'Appearance',
    theme: 'Theme',
    theme_light: 'Light',
    theme_dark: 'Dark',
    theme_system: 'System',
    language: 'Language',
    lang_english: 'English',
    lang_tamil: 'தமிழ்',
    haptics: 'Haptic feedback',
    haptics_desc: 'Vibrate on actions',
    account: 'Account',
    change_password: 'Change password',
    current_password: 'Current password',
    new_password: 'New password',
    confirm_password: 'Confirm new password',
    passwords_no_match: 'Passwords do not match',
    password_changed: 'Password updated successfully',
  },
  ta: {
    // Navigation
    nav_today: 'இன்று',
    nav_customers: 'வாடிக்கையாளர்கள்',
    nav_history: 'வரலாறு',
    nav_me: 'நான்',
    nav_home: 'முகப்பு',
    nav_team: 'குழு',
    nav_reports: 'அறிக்கைகள்',
    nav_settings: 'அமைப்புகள்',

    // Auth
    welcome_back: 'மீண்டும் வரவேற்கிறோம்',
    sign_in_to_continue: 'தொடர உள்நுழையவும்',
    email: 'மின்னஞ்சல்',
    password: 'கடவுச்சொல்',
    sign_in: 'உள்நுழை',
    sign_out: 'வெளியேறு',

    // Common
    save: 'சேமி',
    cancel: 'ரத்து செய்',
    edit: 'திருத்து',
    create: 'உருவாக்கு',
    delete: 'நீக்கு',
    confirm: 'உறுதிப்படுத்து',
    loading: 'ஏற்றுகிறது…',
    error: 'பிழை ஏற்பட்டது',
    no_data: 'தரவு இல்லை',
    back: 'பின்னால்',
    close: 'மூடு',
    refresh: 'புதுப்பி',
    search: 'தேடு',
    filter: 'வடிகட்டு',
    all: 'அனைத்தும்',

    // Loan
    loan: 'கடன்',
    loans: 'கடன்கள்',
    new_loan: 'புதிய கடன்',
    loan_amount: 'கடன் தொகை',
    principal: 'அசல்',
    interest: 'வட்டி',
    disbursed: 'வழங்கியது',
    repayable: 'திரும்பச் செலுத்த வேண்டியது',
    profit: 'லாபம்',
    outstanding: 'நிலுவையுள்ளது',
    installment: 'தவணை',
    installments: 'தவணைகள்',
    start_date: 'தொடக்க தேதி',
    frequency: 'அதிர்வெண்',
    model_a: 'மாதிரி A · முன் கழிப்பு',
    model_b: 'மாதிரி B · கூட்டல்',
    create_loan: 'கடன் உருவாக்கு',
    close_loan: 'கடன் மூடு',
    loan_closed: 'கடன் மூடப்பட்டது',
    assign_collector: 'சேகரிப்பாளர் நியமிக்கவும்',
    reassign: 'மீண்டும் நியமிக்கவும்',
    collections: 'சேகரிப்புகள்',
    schedule: 'அட்டவணை',
    status_active: 'செயலில்',
    status_overdue: 'தாமதமானது',
    status_closed: 'மூடப்பட்டது',
    status_cancelled: 'ரத்து செய்யப்பட்டது',

    // Customer
    customer: 'வாடிக்கையாளர்',
    customers: 'வாடிக்கையாளர்கள்',
    new_customer: 'புதிய வாடிக்கையாளர்',
    full_name: 'முழு பெயர்',
    phone: 'தொலைபேசி',
    address: 'முகவரி',
    risk_level: 'ஆபத்து நிலை',
    risk_low: 'குறைந்த',
    risk_medium: 'நடுத்தரம்',
    risk_high: 'அதிகம்',
    blacklist: 'கருப்பு பட்டியல்',
    unblacklist: 'கருப்பு பட்டியல் நீக்கு',
    blacklisted: 'கருப்பு பட்டியலில் உள்ளது',
    blacklist_reason: 'கருப்பு பட்டியல் காரணம்',
    documents: 'ஆவணங்கள்',
    upload: 'பதிவேற்று',
    id_proof: 'அடையாள சான்று',
    address_proof: 'முகவரி சான்று',
    agreement: 'கையொப்பமிட்ட ஒப்பந்தம்',
    other: 'மற்றவை',

    // Payments
    collect: 'சேகரி',
    collected: 'சேகரிக்கப்பட்டது',
    missed: 'தவறவிட்டது',
    payment_mode: 'கட்டண முறை',
    cash: 'பணம்',
    upi: 'UPI',
    bank: 'வங்கி',
    amount: 'தொகை',
    notes: 'குறிப்புகள்',
    why_missed: 'ஏன் தவறவிட்டீர்கள்?',
    save_collection: 'சேகரிப்பு சேமி',
    mark_as_missed: 'தவறவிட்டதாக குறி',
    customer_unavailable: 'வாடிக்கையாளர் இல்லை',
    customer_refused: 'வாடிக்கையாளர் மறுத்தார்',
    shop_closed: 'கடை மூடியது',
    promised_later: 'பின்னர் கொடுப்பதாக உறுதி',
    wrong_address: 'தவறான முகவரி',
    medical_issue: 'மருத்துவ பிரச்சினை',

    // Dashboard
    net_outstanding: 'நிலுவை மொத்தம்',
    today: 'இன்று',
    overdue: 'தாமதம்',
    collection_trend: 'சேகரிப்பு போக்கு',
    last_30_days: 'கடந்த 30 நாட்கள்',
    top_collectors: 'சிறந்த சேகரிப்பாளர்கள்',
    capital_disbursed: 'வழங்கிய மூலதனம்',
    total_collected: 'மொத்த சேகரிப்பு',
    active_loans: 'செயலில் உள்ள கடன்கள்',
    closed_loans: 'மூடப்பட்டவை',
    my_day: 'என் நாள்',
    target: 'இலக்கு',
    pickups: 'சேகரிக்க வேண்டியவை',

    // Team
    team: 'குழு',
    collectors: 'சேகரிப்பாளர்கள்',
    new_collector: 'புதிய சேகரிப்பாளர்',
    add_collector: 'சேகரிப்பாளர் சேர்',
    visits: 'வருகைகள்',
    collector_name: 'முழு பெயர்',
    collector_email: 'மின்னஞ்சல்',
    collector_phone: 'தொலைபேசி (விருப்பத்தேர்வு)',
    collector_password: 'கடவுச்சொல்',
    create_account: 'கணக்கு உருவாக்கு',

    // Reports
    reports: 'அறிக்கைகள்',
    insights: 'நுண்ணறிவுகள்',
    overdue_loans: 'தாமதமான கடன்கள்',
    nothing_overdue: 'எதுவும் தாமதமாகவில்லை',
    nobody_blacklisted: 'யாரும் பட்டியலில் இல்லை',
    total_interest: 'மொத்த வட்டி',
    avg_loan: 'சராசரி கடன்',
    avg_rate: 'சராசரி வீதம்',

    // Installment status
    status_pending: 'நிலுவையில்',
    status_partial: 'பகுதி',
    status_due_today: 'இன்று செலுத்தவும்',
    status_missed: 'தவறவிட்டது',
    status_replaced: 'மாற்றப்பட்டது',

    // Schedule coverage
    planned: 'திட்டமிட்டது',
    advance_credit: 'முன்பண வரவு',
    covered: 'செலுத்தியது',
    remaining: 'மீதம்',
    carried_forward: 'கடைசியில் சேர்க்கப்பட்டது',
    record_collection: 'பணம் பதிவு செய்',
    total_to_collect: 'வசூலிக்க வேண்டியது',
    expected_per_visit: 'ஒரு வருகைக்கு',
    since: 'முதல்',
    missed_visits: 'தவறிய வருகைகள்',
    collected_so_far: 'வசூலானது',
    collection_date: 'வசூல் தேதி',
    tap_a_bar: 'அந்த நாளைப் பார்க்க பட்டியை அழுத்தவும்',
    no_loans_assigned: 'கடன்கள் எதுவும் ஒதுக்கப்படவில்லை',
    visit_date: 'வருகை தேதி',
    installments_too_many: '10,000 தவணைகளுக்குள் உள்ளிடவும்',
    simple_loan: 'எளிய கடன்',
    simple_loan_desc: 'திட்டம் இல்லை — எந்த நாளிலும் எந்த தொகையும்',
    collect_early_payment: 'முன்கூட்டியே பணம் பெற',
    nothing_left_to_collect: 'இந்த கடன் முழுமையாக வசூலிக்கப்பட்டது',

    // Reschedule
    reschedule: 'மறுதிட்டம்',
    reschedule_schedule: 'மீதமுள்ள திட்டத்தை மாற்று',
    reschedule_mode: 'மீதியை எப்படி வசூலிக்க வேண்டும்?',
    mode_same_end_date: 'அதே முடிவு தேதி',
    mode_same_end_date_desc: 'ஒப்பந்த தேதியில் முடியும்; தினசரி தொகை குறையும்.',
    mode_same_installment: 'அதே தினசரி தொகை',
    mode_same_installment_desc: 'தினசரி தொகை அப்படியே; கடன் சீக்கிரம் முடியும்.',
    reschedule_reason: 'காரணம்',
    reason_advance_payment: 'முன்பணம்',
    reason_customer_request: 'வாடிக்கையாளர் கோரிக்கை',
    reason_missed_payment: 'தவறிய தவணை',
    reschedule_preview: 'மாற்றங்களைப் பார்',
    reschedule_preview_title: 'பழைய திட்டம் → புதிய திட்டம்',
    reschedule_confirm: 'புதிய திட்டத்தை உறுதிசெய்',
    reschedule_done: 'திட்டம் மாற்றப்பட்டது',
    reschedule_failed: 'மாற்ற முடியவில்லை',
    enter_valid_amount: 'பூஜ்யத்திற்கு மேல் தொகையை உள்ளிடவும்',
    reschedule_note: 'செலுத்திய தவணைகளும் வசூல் பதிவுகளும் மாறாது.',
    remaining_balance: 'மீதி தொகை',
    ends: 'முடிவு',
    revision_history: 'மாற்ற வரலாறு',
    revised_by: 'மாற்றியவர்',

    // My Day
    call: 'அழைக்கவும்',
    all_caught_up: 'எல்லாம் முடிந்தது',
    no_pickups_desc: 'இப்போது சேகரிப்பு இல்லை. நல்லது!',
    done: 'முடிந்தது',

    // History
    all_time: 'எல்லா நேரமும்',
    entries: 'பதிவுகள்',
    no_entries_yet: 'இதுவரை பதிவுகள் இல்லை',
    your_collections_desc: 'உங்கள் சேகரிப்புகள் இங்கே தோன்றும்.',

    // Status
    active: 'செயலில்',
    inactive: 'செயலற்றது',

    // Misc
    not_added: 'சேர்க்கப்படவில்லை',
    no_loans_yet: 'இதுவரை கடன்கள் இல்லை',
    no_loans_desc: 'கடன் உருவாக்கியதும் வரலாறு இங்கே தோன்றும்',
    tap_to_view: 'பார்க்க தொட்டுவிடுக',
    missing: 'இல்லை',
    risk: 'ஆபத்து',
    new: 'புதியது',
    no_collectors_yet: 'இதுவரை சேகரிப்பாளர்கள் இல்லை',
    no_collectors_desc: '+ அழுத்தி சேகரிப்பாளர் சேர்க்கவும்',
    nothing_overdue_desc: 'எல்லாம் சரியான நேரத்தில் உள்ளது',
    nobody_blacklisted_desc: 'முதலீட்டாளர் யாரையாவது பட்டியலிட்டால் இங்கே தோன்றும்',
    no_collections_yet: 'இதுவரை சேகரிப்புகள் இல்லை',
    no_collections_desc: 'சேகரிப்பாளர்கள் பணம் பெற்றதும் செயல்திறன் தோன்றும்',
    lifetime: 'ஆயுள்காலம்',
    needs_attention: 'கவனம் தேவை',
    create_customer: 'வாடிக்கையாளர் உருவாக்கு',
    save_changes: 'மாற்றங்கள் சேமி',
    collector: 'சேகரிப்பாளர்',
    name: 'பெயர்',
    missed_visit: 'வருகை தவறவிட்டது',

    // Collector detail
    deactivate: 'நிறுத்து',
    activate: 'செயல்படுத்து',
    delete_collector: 'சேகரிப்பாளரை நீக்கு',
    delete_customer: 'வாடிக்கையாளரை நீக்கு',
    confirm_delete: 'நீக்கலை உறுதிப்படுத்து',
    delete_collector_warn: 'இந்த சேகரிப்பாளரை நிரந்தரமாக நீக்கும். முதலில் கடன்களை மாற்றவும்.',
    delete_customer_warn: 'இந்த வாடிக்கையாளரை நிரந்தரமாக நீக்கும். முதலில் கடன்களை மூடவும்.',
    cannot_undo: 'இதை மாற்ற முடியாது.',
    recovery_rate: 'மீட்பு விகிதம்',
    no_trend_data: 'இதுவரை சேகரிப்புகள் இல்லை',
    collection_rate: 'சேகரிப்பு விகிதம்',
    high_risk: 'அதிக ஆபத்து',
    sort_by: 'வரிசை',
    sort_amount: 'தொகை',
    sort_installments: 'தவணைகள்',
    this_week: 'இந்த வாரம்',
    this_month: 'இந்த மாதம்',
    this_year: 'இந்த ஆண்டு',
    period: 'காலம்',
    collector_filter: 'சேகரிப்பாளர்',

    // Profile
    profile: 'சுயவிவரம்',

    // Settings
    settings: 'அமைப்புகள்',
    appearance: 'தோற்றம்',
    theme: 'கருப்பொருள்',
    theme_light: 'வெளிர்',
    theme_dark: 'இருள்',
    theme_system: 'கணினி',
    language: 'மொழி',
    lang_english: 'English',
    lang_tamil: 'தமிழ்',
    haptics: 'அதிர்வு கருத்துரை',
    haptics_desc: 'செயல்களில் அதிர்வு',
    account: 'கணக்கு',
    change_password: 'கடவுச்சொல் மாற்று',
    current_password: 'தற்போதைய கடவுச்சொல்',
    new_password: 'புதிய கடவுச்சொல்',
    confirm_password: 'புதிய கடவுச்சொல் உறுதிப்படுத்து',
    passwords_no_match: 'கடவுச்சொற்கள் பொருந்தவில்லை',
    password_changed: 'கடவுச்சொல் வெற்றிகரமாக மாற்றப்பட்டது',
  },
} as const;

export type TranslationKey = keyof typeof translations.en;
